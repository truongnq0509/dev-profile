...
import { getCategoryPost, getCategoryPosts } from "../../../services/categoryPostService"
...
const PostPage = () => {
	...
	const [title, setTitle] = useTitle("Truong Nguyen | Tất Cả Bài Viết")

	useEffect(() => {
		const fetchApi = async () => {
			const [res1, res2] = await Promise.all([getCategoryPosts(), getPosts()])
			setCategories(res1)
			setProjects(res2)
		}
		fetchApi()
		...
		cateElement?.forEach((item) => {
			item.addEventListener("click", async (e) => {
				...
				if (!id) {
					...
					const response = await getCategoryPost(id)
				...
				<div class="row">
					${projects?.map((project) => {
						return `
							<div class="col-4 py-5">
								<a class="${cx("cart")}" href="#/post/${project?.id}">
									...
									<div class="${cx("cart__desc")}">
										<h3>${project?.title}</h3>
										<span>${project?.categoryPost?.name || active?.name}</span>
									</div>
								</a>
							</div>
						`
					...
					`
						<div class="${cx("not")}">
							...
							<span>Không có bài viết nào liên quan</span>
						</div>
						`}
				</div>
			</div>
