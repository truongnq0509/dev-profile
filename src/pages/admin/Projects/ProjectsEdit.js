...
import { updateProject as apiUpdateProject, getProject as apiGetProject } from "../../../services/projectService"
		...
		form.addEventListener("submit", async (e) => {
			...
			if (form) {
				...
				for (let [key, value] of formData) {
					newProject = { ...newProject, [key]: value }
				}
			}
								...
								>
									${categories?.map((category, index) => {
										return `
												...
												${category?.id == project?.categoryProjectId ? `selected` : ""} >${category.name}</option>
											`
									})}
								</select>
							</div>
