...
const AddPostsPage = () => {
	...
	useEffect(() => {
		...
		const handlBlur = async (e) => {
			...
			setLocalStorage("PROJECT", project)
		...
		inputs.forEach((input) => {
			input.addEventListener("blur", handlBlur)
		})

		textarea.addEventListener("blur", handlBlur)
						...
						<div class="row">
							...
							<div class="col-12">
								<textarea
									...
									placeholder="Mô Tả"
								>
${getLocalStorage("PROJECT")?.description}
								</textarea
								>
								${errors?.description &&
								...
								>
									${categories.map((category, index) => {
										return `
												...
												${getLocalStorage("PROJECT")?.categoryProjectId == category.id ? `selected` : ""} >${category.name}</option>
											`
									})}
								</select>
							</div>
